...
int c_toybox(char *argv[])
{
  ...
  if (!sigsetjmp(rebound, 1)) {
    ...
    toy_singleinit(tl, argv);
    tl->toy_main();
    xexit();
  }
  toys.rebound = 0;
  int exitval = toys.exitval;
  clearerr(stdout);
 ...
 */
const struct builtin toybuiltins [] = {
#undef NEWTOY
#undef OLDTOY
#define NEWTOY(name, opts, flags) {#name, c_toybox},
#define OLDTOY(name, oldname, flags) {#name, c_toybox},
#include "../toybox/generated/newtoys.h"
	{NULL, NULL}
};
